...
class XFormersAttnProcessor:
    ...
    def __call__(
        ...
        query = attn.to_q(hidden_states)  # [bs, HW, xx]
        ...
        key = attn.to_k(encoder_hidden_states)  # [bs, HW/77, xx]
        value = attn.to_v(encoder_hidden_states)  # [bs, HW/77, xx]
        
        ###### kailai code ######
        atten = None
        # {image: q[1, 4096, 512] & k[1, 4096, 512], atten: q[bs, 4096/1024/256/64, xx] & k[bs, 77, xx]}
        q, k = query.clone().detach().float(), key.clone().detach().float()
        # if q.shape[0] != 1:
        atten = attn.scale * (q @ k.transpose(-1, -2))  # [bs, HW, 77]

        query = attn.head_to_batch_dim(query).contiguous()  # [bs*xx/64, HW, 64]
        key = attn.head_to_batch_dim(key).contiguous()  # [bs*xx/64, 77, 64]
        value = attn.head_to_batch_dim(value).contiguous()  # [bs*xx/64, 77, 64]
