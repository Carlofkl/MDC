...
class UNetMidBlock2D(nn.Module):
    ...
    def forward(self, hidden_states, temb=None):
        ...
        for attn, resnet in zip(self.attentions, self.resnets[1:]):
            if attn is not None:
                hidden_states, atten = attn(hidden_states, temb=temb)
            hidden_states = resnet(hidden_states, temb)
            if isinstance(atten, torch.Tensor):
                return hidden_states, atten
            elif isinstance(atten, dict) and atten is not None:
                attens['self'].extend(atten['self'])
                attens['cros'].extend(atten['cros'])
            else:
                continue

        return hidden_states, attens
...
class AttnDownBlock2D(nn.Module):
    ...
    def forward(self, hidden_states, temb=None, upsample_size=None):
        ...
        for resnet, attn in zip(self.resnets, self.attentions):
            hidden_states = resnet(hidden_states, temb)
            hidden_states, atten = attn(hidden_states)
        ...
        return hidden_states, output_states, attens
    ...
    def forward(
        ...
        for resnet, attn in zip(self.resnets, self.attentions):
            if self.training and self.gradient_checkpointing:
                ...
                hidden_states = resnet(hidden_states, temb)
                hidden_states, atten = attn(  # atten maybe []?
                    hidden_states,
                    encoder_hidden_states=encoder_hidden_states,
                    cross_attention_kwargs=cross_attention_kwargs,
                    attention_mask=attention_mask,
...
class AttnSkipDownBlock2D(nn.Module):
    ...
    def forward(self, hidden_states, temb=None, skip_sample=None):
        ...
        return hidden_states, output_states, skip_sample, attens
